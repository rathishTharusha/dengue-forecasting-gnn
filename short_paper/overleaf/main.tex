...
% No model name. Earlier drafts carried one (PIAG-Net, then GAG-Net); the paper
% no longer proposes an architecture, so naming one would misdescribe it. What is
% proposed is an evaluation protocol and a set of measurements.
...
% TITLE. The paper reports what caps accuracy on this task and shows why a
% mechanistic prior cannot lift it. It does not propose an architecture, so the
% title does not name one. Earlier drafts were titled for a model (PIAG-Net, then
% GAG-Net); both names outlived the claim they described.
\title{Know When the Epidemic Comes:\\Mathematics Before Data in Dengue
...
\keywords{dengue forecasting, graph neural networks, reproducibility,
evaluation protocols, physics-informed learning, epidemic early warning,
negative results}
...
% Table floats can only move FORWARD, so the input sits early: declared late it
% landed on the references page, and each step earlier lets it settle sooner and
% frees the text after it to reflow.
