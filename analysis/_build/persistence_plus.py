"""Two ways to make the mechanistic correction to persistence safer.
...
ARMS = {
    "v2":          dict(memory=0,   combine="none"),
    "waning_26w":  dict(memory=26,  combine="none"),
    "waning_52w":  dict(memory=52,  combine="none"),
    "waning_104w": dict(memory=104, combine="none"),
    "capped":      dict(memory=0,   combine="capped"),
    "damped":      dict(memory=0,   combine="damped"),
    "waning52_capped": dict(memory=52, combine="capped"),
}


def initial_state(cases, population, idx, memory: int):
    """Compartment fractions. `memory` weeks of past infection deplete S; 0 = none."""
    ...
    onset_e = c2 / (RHO * pop)
    if memory > 0:
        recent = np.stack([np.nansum(cases[max(0, i - memory): i], axis=0) for i in idx])
