...
class Net(nn.Module):
    ...
    def forward(self, x: torch.Tensor, fixed: torch.Tensor, p_z: torch.Tensor,
                ...
                edge: torch.Tensor | None = None) -> torch.Tensor:
        """Return fold-scaled log1p predictions ``(B, N, H)`` for every head.
        ...
        incidence back into that space before returning.
        """
        ...
        return p_z + gate * (phys_z - p_z)


def seir_state(cases_raw: torch.Tensor, pop: torch.Tensor, cum: torch.Tensor,
               rho: float = 1.0 / 11.0, s0: float = 1.0 - 0.682) -> torch.Tensor:
    ...
    """
    i0 = (cases_raw[..., -1] / (rho * pop)).clamp(1e-6, 0.5)
    e0 = (cases_raw[..., -2] / (rho * pop)).clamp(1e-6, 0.5)
    s = (s0 - cum / (rho * pop)).clamp(0.01, 1.0)
